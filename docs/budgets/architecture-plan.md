...
## 9. Incorporating budget-design.md (architecture delta)

`budget-design.md` reframes the budget as a **rules engine over a volatile macro regime** — household floors from official baskets, a strategy *recommendation* engine, trigger-based rebalancing, scenario simulation, and a `MacroSnapshot`/provenance/confidence layer fed by official sources (INDEC/BCRA/ARCA). The genuinely-new, cheap, feed-free parts fold into the MVP; the macro-feed and rules-engine machinery is Phase 2/3 because the official-data feeds are the exact maintenance/legal cost the owner is still researching. **Guiding principle: keep the MVP lean, official-data feeds are suggestion-only and never hardcoded, nothing changes silently.** The deliverable's §7.1 category set + §7.2 phase decisions are the spec; this section says *how*.

### 9.1 MVP additions (cheap, feed-free) — all ride on the §2.1 `budget_income` table + pure functions

- **9.1.1 Household floor** — a per-month `floor_amount` Decimal co-located on the `budget_income` row (+ `floor_source 'manual'|'computed'`), NOT a new `BudgetPlan` table or settings scalar (it's per-period + read with income). Populate **manually** (user types essentials floor) or **computed** = `Σ(kind='spend' targets WHERE is_essential)` via a pure `compute_floor(spend_lines, is_essential)`. CBT/`canasta de crianza` auto-fetch is Phase 3 (feed cost). Reader exposes `income` + `floor` together (essentials-floor-vs-income readout).
- **9.1.2 Strategy suggestion** — pure `suggest_strategy(income, floor, debt_min)` using adequacy (`income/floor`) + debt-ratio → conservative/balanced/aggressive. The reduced, feed-free subset of budget-design.md's `choose_strategy`: **drop** volatility (needs 6-mo history) + FX/wage-gap (need MacroSnapshot) → Phase 2/3. `debt_min` is a manual UI field (not persisted; YAGNI). Suggestion only — user picks; no `Recommendation` entity in MVP.
- **9.1.3 Income-pressure segment** — pure `income_pressure(income, floor)` → Constrained `<1.3×` / Stable `1.3–2.5×` / Comfortable `>2.5×` (ratio-to-floor, not nominal bands). Calm readout + feeds the suggestion.
- **9.1.4 Floor-before-percentages guard** — a fix to §2.3's `apply_saving_profile`: pure `floor_guard(income, floor, saving_total)`; if savings would push essentials below the floor it still writes the rows but returns `floor_breached: true` (+ gap) → UI warns ("consider Conservative"). **Warns, never silently rebalances** (the active `fund_gap_from_nonessential_buckets` is deferred).
- **9.1.5 Category set + `is_essential`** (LOCKED 2026-06-30) — MVP delta: `Rent`→`Housing` (keep `Rent` alias) + `Education`; Phase-2: split `Services`→`Utilities`, add `Social` (dining/outings), `DebtService`, `FamilySupport` (tolerant strings, no migration). **Essentiality = an `ESSENTIAL_CATEGORIES` code constant** (`is_essential(cat)`; essentials = Housing/Utilities/Food/Transport/Health/Education/Taxes/DebtService) — zero schema; per-user override deferred (Phase 2). Dropped-because-covered (Savings ARS/USD, Investments, Dollarized, Cash/informal, FX-purchases) → Accounts/Transfers/per-account currency. **Note:** the **monotributo cuota is NOT dropped** — it's a `Taxes` expense; the Monotributo module separately tracks the income standing. `Taxes` covers monotributo cuota · autónomos · Ganancias · IIBB (AGIP/ARBA) · ABL/municipal.

### 9.2 Phase 2/3 sketches (map each budget-design.md entity to an existing primitive; why NOT MVP)

| budget-design.md element | Cosmic-python shape (Phase) | Reuses / why NOT MVP |
|---|---|---|
| **MacroSnapshot** (cpi/wage/fx official+mep+ccl+blue/rates/subsidy+tax versions) | Immutable append-only aggregate keyed `(period, source, captured_at)`, à la Monotributo snapshot history (ADR-052) | Reuses snapshot-history precedent. NOT MVP: presupposes official feeds (the open cost); MVP's manual inflation % (§2.4) stands in. |
| **Trigger-based rebalancing** (overspend>10%×2mo, cpi accel>1pp, fx shock>5%, subsidy/tax version change → cap_pp_change) | Pure trigger eval `(actuals, macro_now, macro_prev)→triggers` + a prompted `propose_rebalance` preview (capped) | Reuses §2.4 reprice as the apply step + ADR-042 actuals. NOT MVP: needs MacroSnapshot deltas. |
| **Multi-currency `rate_type`** (official/mep/ccl/blue) | Tolerant-string tag on budget lines/transactions, riding per-account currency (ADR-123/134) + `FxRateType` (ADR-044) | NOT MVP: budget is ARS-only (ADR-125); nothing to qualify yet. |
| **Scenario** (shock_params/severity) | Simulation service extending the Forecast pace-projector (ADR-129); read-only projection | NOT MVP: forward-modeling = the Forecast slice (#53). |
| **Recommendation + provenance/confidence + source-priority/fallback** | Provenance value object (`source, confidence, captured_at`) + append-only `Recommendation` log; pure source-priority resolver (official→provincial→media→private) | Reuses snapshot `source` + dolarapi suggest/confirm (ADR-044). NOT MVP: no auto-derived figures to attribute. |
| **Override governance** (`reason_code, source_snapshot, expires_at`; never silent) | Append-only override log + `expires_at` on auto-applied params + confidence badges/confirmation | Reuses the "prompted, never silent" stance (§2.4/ADR-044). NOT MVP: no automation to override. |

**Governance carried forward:** official feeds (INDEC/BCRA/ARCA) are suggestion-only, never auto-applied, never hardcoded (ADR-044 epistemic hygiene); fallback order + "hold last trusted value, label unofficial" recorded in the Phase-3 source-priority ADR. **The owner's open question stands** — whether to take on any official feed at all; this plan keeps every macro input manual until that lands.

### 9.3 MVP delta files (additional to §8)
Backend: `budget_income` gains `floor_amount`/`floor_source` (same MVP migration); new pure `domain/models/strategy.py` (`suggest_strategy`, `income_pressure`, `floor_guard`) + `compute_floor`/`ESSENTIAL_CATEGORIES`/`is_essential` in `value_objects.py`; `apply_saving_profile` returns `floor_breached`; reader exposes `floor`/`pressure`/`suggestedStrategy`. Frontend: `NetIncomeHeader` gains floor field + pressure readout; profile picker shows suggested strategy + floor-breach warning; math in `derive.ts`. Tests: unit (`suggest_strategy` branches, `income_pressure` boundaries 1.3/2.5, `floor_guard`, `compute_floor`) + e2e (income+floor round-trip, profile-apply floor-breach flag); 100% gate.

---

## 10. Revised ADR list (supersedes §7)

Keep ADR-137…142 with the edits below, plus two new (143, 144). **8 ADRs total.**

1. **ADR-137 — Inflation-reprice model** *(architecture)*: pure `cap×(1+infl)+step_up`, only `kind='spend'`, confirm-at-rollover. *Rejected:* silent auto-reprice.
2. **ADR-138 — Saving-profile presets as code constants; savings as `kind='saving'` rows** *(architecture)*. **Edit:** `apply_saving_profile` runs the floor-before-% guard (§9.1.4) — warns (`floor_breached`), never silently rebalances. *Rejected:* dedicated savings table in MVP; an allocation solver that overrides the chosen profile.
3. **ADR-139 — Net-income base + household floor as a per-month `budget_income` row; variable base is a suggestion** *(data)*. **Edit:** row also carries `floor_amount`/`floor_source` (manual or computed=Σ essential targets). *Rejected:* a `BudgetPlan` aggregate in MVP (presupposes snapshots/versioning); a settings scalar (wrong cardinality).
4. **ADR-140 — Final reconciled budget-category set (LOCKED) + `is_essential` as a code constant** *(data)*. Adopt the locked §7.1 set (Housing·Utilities·Food·Social·Transport·Health·Education·Shopping·Entertainment·Subscriptions·Taxes·DebtService·FamilySupport·Fees +Income/Other); MVP=`Rent`→`Housing`(alias)+`Education`; Phase-2=split`Services`→`Utilities`, add `Social`/`DebtService`/`FamilySupport` (tolerant strings). Monotributo cuota → `Taxes` (the module tracks standing). `ESSENTIAL_CATEGORIES` constant; per-user override deferred. Record dropped-because-covered rationale. *Rejected:* per-user `Category` aggregate in MVP (YAGNI); destructive `Rent` rename; monotributo cuota as a dropped/non-category.
5. **ADR-141 — Macro inputs are manual + suggestion-only; no official-data scraping (INDEC/BCRA/ARCA) in MVP** *(risks)*. **Edit:** broadened from inflation-only to all official feeds — suggestion-only, never auto-applied/hardcoded; the owner's open maintenance question. *Rejected:* live official scraping in MVP (no clean APIs; ToS/ops fragility).
6. **ADR-142 (Phase 2 placeholder) — Savings buckets = first-class aggregate funded by real Transfers; bucket = view over its account** *(architecture)*. *Rejected:* a reconciling bucket balance.
7. **ADR-143 — Household-floor readout + pure strategy suggestion (manual-floor MVP; macro-scored selector deferred)** *(architecture)*: feed-free `suggest_strategy`(adequacy+debt-ratio) + `income_pressure` ratio-to-floor *suggest*; user picks. Volatility/FX/wage-gap + full `choose_strategy` → Phase 2/3 (need history + MacroSnapshot). *Rejected:* full macro-scored selector in MVP; nominal income bands.
8. **ADR-144 (Phase 2/3 placeholder) — Macro rules-engine layer: immutable `MacroSnapshot`, trigger-based rebalancing, provenance/confidence + source-priority fallback, Override governance** *(architecture)*: append-only snapshot (recalc without mutating history, ADR-052) → prompted capped rebalancer; provenance on every macro figure (official→provincial→media→private); append-only override log; nothing silent (ADR-044). Scenario sim rides on Forecast (ADR-129) at Phase 3. *Rejected:* hardcoding/auto-applying feeds; mutating snapshots in place.
