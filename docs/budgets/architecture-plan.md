...
- **9.1.5 Category set + `is_essential`** — MVP delta: `Rent`→`Housing` (keep `Rent` alias) + `Education`; Phase-2: `Utilities`/`EatingOut`/`DebtService`/`FamilySupport` (tolerant strings, no migration). **Essentiality = an `ESSENTIAL_CATEGORIES` code constant** (`is_essential(cat)`; essentials = Housing/Utilities/Food/Transport/Health/Education/DebtService) — zero schema; per-user override deferred (Phase 2). Dropped-because-covered (Savings ARS/USD, Investments, Dollarized, Cash/informal, FX-purchases, Monotributo cuota) recorded as rationale → Accounts/Transfers/per-account currency/Monotributo.
...
4. **ADR-140 — Final reconciled budget-category set + `is_essential` as a code constant** *(data)*. Adopt §7.1 set; MVP=`Rent`→`Housing`(alias)+`Education`; Phase-2=`Utilities`/`EatingOut`/`DebtService`/`FamilySupport` (tolerant strings). `ESSENTIAL_CATEGORIES` constant; per-user override deferred. Record dropped-because-covered rationale. *Rejected:* per-user `Category` aggregate in MVP (YAGNI); destructive `Rent` rename.
