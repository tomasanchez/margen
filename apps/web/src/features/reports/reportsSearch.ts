 ...
 * window lives in the URL as `?range=6M` (the default 3M is omitted) so reload,
 ...
 *    to `3M` when `range` is absent/invalid (ADR-169);
 *  - {@link rangeToReportsSearch} encodes a range back, OMITTING the default `3M`
...
export const DEFAULT_REPORTS_RANGE: ReportsRange = '3M'
 ...
 * default 3M window". Anything that is not one of the four presets is dropped so
 ...
 * resolves to the default {@link DEFAULT_REPORTS_RANGE} (`3M`).
 */
...
/**
 * Encode a range back into `/reports` search params, OMITTING the default `3M`
