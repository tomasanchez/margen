"""Unit tests for the reports-queries adapter helpers (ADR-164, ADR-025).

The reader's cumulative SQL aggregation is covered end to end by the e2e tier on
in-memory SQLite (and the real-Postgres integration tier); this module covers the
pure ``_as_decimal`` coercion that protects against a driver returning a float for
a summed money column (ADR-025).
"""
...
from margen_api.adapters.reports_queries import _as_decimal
