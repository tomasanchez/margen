...
The **Web** job also runs `npm audit --audit-level=high` after `npm ci` as a blocking step, so a high/critical dependency advisory fails the build before it can reach `main` (enforcing ADR-022). Moderate/low advisories are intentionally not gated to avoid noise from transitive dev dependencies; tighten to `--audit-level=moderate` if the team wants a stricter bar.
